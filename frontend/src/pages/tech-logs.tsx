...
export default function TechLogs() {
  ...
  const { data: posts, isLoading, error } = useQuery({
    queryKey: ["/api/posts", "tech", tagParam],
    queryFn: () => api.listPosts("tech", tagParam || undefined),
  });
    ...
    <div className="container mx-auto px-6 lg:px-8 py-12 md:py-16">
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-8">
            <div className="text-xs font-bold uppercase tracking-wider text-primary mb-2">// tech_dev_logs</div>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
                  {tagParam ? (
                    <>Tech posts tagged <span className="text-primary">#{tagParam}</span></>
                  ) : (
                    "Tech Chapters"
                  )}
                </h1>
                <p className="text-xs text-muted-foreground mt-1">Dev logs, code retrospective breakdowns, and engineering details.</p>
              </div>
          ...
          {error ? (
            <div className="py-12 px-6 text-center border border-destructive/20 rounded-xl bg-destructive/5 text-destructive flex flex-col items-center gap-2">
              ...
              <p className="text-sm font-semibold">Failed to fetch tech chapters</p>
            ...
            <div className="py-20 flex flex-col items-center text-center border border-dashed border-border/50 rounded-xl bg-card/30">
              ...
              <p className="text-sm text-muted-foreground">No tech posts found.</p>
